...
import { PLANET_DATA, isStar, type Planet } from '../data/planets';
...
export function createPlanets(
  ...
  return PLANET_DATA.map((d: Planet): PlanetInstance => {
    ...
    for (const m of MOONS_BY_PARENT.get(d.name) ?? []) {
      let moonMat: THREE.Material;
      if (m.file) {
        moonMat = new THREE.MeshStandardMaterial({
          map: loadColorTexture(tl, m.file),
          roughness: 0.9,
        });
      } else {
        moonMat = new THREE.MeshStandardMaterial({
          color: m.color ?? 0x999999,
          roughness: 0.9,
        });
      }
      ...
      moonMesh.position.set(
        Math.cos(phase) * m.orbitRadius,
        (m.tilt ?? 0) * m.orbitRadius,
        Math.sin(phase) * m.orbitRadius,
      );
      mesh.add(moonMesh);
      moons.push({
        mesh: moonMesh,
    ...
    if (d.dist > 0) {
      const oc = new THREE.EllipseCurve(
        0,
        0,
        d.dist,
        d.dist,
